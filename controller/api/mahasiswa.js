...
exports.allMhs = async (req, res) => {
  ...
  let queryExe = {
    nim: IsIncludes(nim + ""),
    nama: IsIncludes(nama),
    jurusan: IsIncludes(jurusan),
    kelas: IsIncludes(kelas),
  };

  var totalMhs = await mahasiswa.count({}, (err, count) => count);
  var allMhsShow = await mahasiswa
    .find(queryExe)
    .sort({ createdAt: -1 })
    .skip(page)
    .limit(limit);
...
exports.addMhs = async (req, res) => {
  ...
  if (req.body.jurusan === "teknik informatika") {
    Nim = await mahasiswa
      .find({ jurusan: "teknik informatika" })
      .sort({ nim: -1 })
      .limit(1);
...
exports.updateMhs = async (req, res) => {
  var mhs = await mahasiswa.findOneAndUpdate(
